...
def d_scores(dtc_model, X_train, y_train):
    ...
    figx, ax = plt.subplots(1,1, figsize=(10,8))
    ConfusionMatrixDisplay.from_estimator(dtc_model, X_train, y_train, ax=ax)
    figx.savefig("confusion.png")

    return prediction_df, prediction_scores, figx
...
def d_evaluate(dtc_model, X_train):
    ...
    figx.savefig("bar.png")

    figx1, ax = plt.subplots(1,1, figsize=(20,5))
    plot_tree(dtc_model, feature_names=X_train.columns, class_names=['healthy', 'disease'], filled=True, ax=ax)
    figx1.savefig("tree.png")
    return eval_df, figx, figx1
...
def val_curve(dtc_model, X_train, y_train):
    ...
    figx.savefig("val_curve.png")
    return val_summary_df, figx
...
def pr_curve(dtc_model, X_train, y_train):
    ...
    figx.savefig("precision_recall.png")
    return avg_pre_recall, figx
...
def lr_curve(dtc_model, X_train, y_train):
    ...
    figx.savefig("learning_curve.png")
    return train_size, train_scores, test_scores, figx
...
if __name__ == "__main__":
    ...
    prediction_df, prediction_scores, figx = d_scores(dtc_model, X_train, y_train)
    eval_df, figx, figx1 = d_evaluate(dtc_model, X_train)
    roc, figx = roc_auc(dtc_model, X_train, y_train)
    val_summary_df, figx = val_curve(dtc_model, X_train, y_train)
    avg_pre_recall, figx = pr_curve(dtc_model, X_train, y_train)

    train_size, train_scores, test_scores, figx = lr_curve(dtc_model, X_train, y_train)
    figx = calibration(dtc_model, X_train, y_train)
